function varargout = button42(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @button42_OpeningFcn, ...
    'gui_OutputFcn',  @button42_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function button42_OpeningFcn(hObject, eventdata, handles, varargin)
...
X = imread('箭頭.jpg');
% axes(handles.axes1)
set(handles.axes1,'position',[96,26,16,4]);
imshow(X)
sec15 = 1;
t = timer('TimerFcn',{@t_TimerFcn,hObject},'Period',1,'ExecutionMode','fixedRate', 'TasksToExecute',16);
guidata(hObject, handles);

function varargout = button42_OutputFcn(hObject, eventdata, handles)
...
if a(1) || a(2) || a(3) == 1
    ...
    if n < 3
        if mod(color,2)==0
            ...
            set(handles.axes1,'position',[96,22,16,4])
        end
        ...
        for ii = 1:4
            middleofboom(judgein,color1,hObject, eventdata, handles,pbnumber1,near,clrin,ii);
        end
    end
    clrhelp = 1;
...
function middleofboom(judgein,color1,hObject, eventdata, handles,pbnumber1,near,clrin,ii)
